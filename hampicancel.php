...
if (isset($_POST['id'])) {
    ...
    $check = "SELECT * FROM hampi WHERE id = $id";
    $res = $conn->query($check);

    if ($res->num_rows > 0) {
        // Booking exists — delete it
        $sql = "DELETE FROM hampi WHERE id = $id";
        if ($conn->query($sql) === TRUE) {
            // Reorder IDs to stay continuous
            $conn->query("SET @count = 0");
            $conn->query("UPDATE hampi SET id = @count:=@count+1");
            $conn->query("ALTER TABLE hampi AUTO_INCREMENT = 1");
